...
class Vector
{
    public:
        ...
        inline Vector   subtract(Vector* b)   {return Vector(x-b->x,y-b->y,z-b->z);}
        inline Vector   subtract(Vector& b)   {return Vector(x-b.x,y-b.y,z-b.z);}
        inline Vector   cross(Vector* b)      {return Vector(y*(b->z)-z*(b->y),z*(b->x)-x*(b->z),x*(b->y)-y*(b->x));}
        inline Vector   cross(Vector& b)      {return Vector(y*b.z-z*b.y,z*b.x-x*b.z,x*b.y-y*b.x);}
        inline double   dot(Vector* b)        {return x*b->x+y*b->y+z*b->z;}
        inline double   dot(Vector& b)        {return x*b.x+y*b.y+z*b.z;}
        double   dot(Vertex& b);        
        double   dot(Vertex* b);        
